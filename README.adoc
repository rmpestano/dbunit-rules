...
DataSet Configuration:: this basically setup the `dataset` which will be used. The only way to configure a dataset is using *@DataSet* or *@UsingDataSet (in case of CDI) annotation.
+
...
----
     @Test
     @DataSet(value ="users.yml", strategy = SeedStrategy.UPDATE,disableConstraints = true,cleanAfter = true,transactional = true)
...
DBUnit Configuration:: this basically setup the `dbunit`. It can be configured by `@DBUnit` annotation (class or method level) and `dbunit.yml` file present in test resources folder.
+
[source,java]
----
    @Test
