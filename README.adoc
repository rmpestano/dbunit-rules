...
NOTE: https://github.com/rmpestano/dbunit-rules/blob/master/core/src/main/java/com/github/dbunit/rules/util/EntityManagerProvider.java[EntityManagerProvider^] is a simple JUnit rule that creates a JPA entityManager for each test. Dbunit rule don't depend on EntityManagerProvider, it only needs a JDBC connection;

[TIP]
====
You can https://github.com/junit-team/junit4/wiki/rules#rulechain[chain^] both rules to guarantee execution order:
