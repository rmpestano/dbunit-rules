...
|[red]#*failed*#
12+^|*Totals*
|8|1|9|32|1|0|0|0|0|33 2+|04s 142ms
|===
...
Given ::
=====
DBUnit interceptor is enabled in your test beans.xml: icon:thumbs-up[role="green",title="Passed"] [small right]#(03s 573ms)#
******
...
Then ::
=====
Dataset script should be interpreted while seeding the database icon:thumbs-up[role="green",title="Passed"] [small right]#(002ms)#
=====
****
...
Then ::
=====
Test must pass because database state is as in expected dataset. icon:thumbs-up[role="green",title="Passed"] [small right]#(001ms)#
=====
****
...
Then ::
=====
Test must fail showing what it was expecting as database state. icon:thumbs-down[role="red",title="Failed"] [small right]#(545ms)#
******
...
IMPORTANT: junit.framework.ComparisonFailure: value (table=USER, row=0, col=name) expected:<[]expected user1> but was:<[non ]expected user1>
